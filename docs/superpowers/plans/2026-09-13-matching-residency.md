...
- [x] Freeze binary and source. Run fresh paired current/features/window CPU and CUDA controls, two reversed repeats on first 250 stairs scans. Compare frozen previous CUDA separately with alternating order and trace agreement.
- [x] Run complete stairs validation, profile final pipeline, document timings and correctness with stage/coverage labels. Commit final report and completed plan.
...
Search milestone `d76f359`: exact result hashes across block sizes 64/128/256/512; chosen 64 gave 2.1–4.3x synthetic kernel speedups. Hardware memory counters unavailable (`ERR_NVGPUCTRPERM`). Two reversed first-100-scan replay repeats showed no reliable total gain (baseline 42.07/38.73 ms, search 40.75/41.97 ms); this is a diagnostic, not the final comparison. Deferred storage milestone `d325e66`: serial/concurrent raw reads, copies, cancellation, append, and exception retry tested.

Core `24c6da0` and adapter `8a4af10` pass 86 CUDA C++ tests; CPU-only 42 pass with 8 CUDA skips; 30 Python tests pass. Matcher/adapter 18 tests pass memcheck, synccheck and racecheck with zero errors. Frozen initial combined implementation: `benchmarks/results/matching-residency-backend/form-replay`, SHA256 `77d7a1f297aeb09d932d53149c05ee5173ac4ca00cb3416139964b0ab140485f`.

Final follow-up `ae92335` reuses uniquely owned snapshots, batches and staging, and fixes failed-host-reset recovery (runtime failing regression before the guard). Final verification: 87 CUDA tests, 42 CPU-only tests with 8 skips, 30 Python tests; six updated adapter tests pass memcheck/racecheck. Final 24-run scaling suite `matching-reuse-pilot` and 6-run full 1190-scan suite `matching-reuse-stairs-full` completed. All final trace workload/iteration counts agree. Full quality gates pass; final CUDA max position difference <1.18e-11 m.

Full-sequence mean total: original 155.76, previous CUDA 70.80, final CUDA 50.64 ms/scan (3.08x original, 1.40x incremental). Final scaling totals current/features/window 33.90/62.42/58.31 ms with identical-workload CPU and previous CUDA controls. Detailed stages, profile evidence, scope and limitations: `docs/cuda-matching-residency-results.md`. Preparation has mixed stage-level effects; the full map stage rises 0.19 ms while total falls 20.16 ms. All three approved changes are implemented; no global-optimality claim is made.
