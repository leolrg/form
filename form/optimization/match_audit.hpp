...
public:
  MatchAudit() {
    ...
    if(stats_path && *stats_path) {
      ...
      stats_ << "scan,kind,total,certified,searched,cell_fallback,gap_fallback,unchanged,mismatches,oracle_searched,active_leaves,dirty_leaves,summary_checks,relative_gram_error,split_queries,summary_tree,summary_bounded\n";
    }
  ...
  void appendStats(size_t scan,int kind,const CudaMatcher::ReuseStats& s,const CudaMatcher::SummaryStats& q) {
    ...
    stats_ << scan << ',' << kind << ',' << s.total << ',' << s.certified << ',' << s.searched
           ...
           << ',' << q.relative_gram_error << ',' << s.split_queries << ',' << int(q.tree) << ',' << int(q.bounded) << '\n';
    stats_.flush();
  }
