  ...
  try {
    ...
    for (int i = 1; i < argc; ++i) {
      ...
      if (arg == "--input") input = value;
        ...
        if (value != "reference" && value != "summary" && value != "cuda" && value != "summary-batch" && value != "cuda-batch" && value != "summary-resident" && value != "cuda-resident") throw std::runtime_error("Unknown backend " + value);
        params.constraints.use_summary = value != "reference";
        params.constraints.use_cuda_summaries = value == "cuda" || value == "cuda-batch" || value == "cuda-resident";
        params.constraints.use_resident_optimizer = value == "summary-resident" || value == "cuda-resident";
