...
def main():
    ...
    parser.add_argument('--modes', nargs='+', choices=('baseline', 'off', 'audit', 'certified', 'cpu'), default=['off', 'certified'])
    ...
    if 'audit' in args.modes and not args.diagnostic:
    ...
    with (ROOT/'benchmarks/results/.rematching.lock').open('a') as lock:
        ...
        for key in ('FORM_CUDA_MATCH_REUSE', 'FORM_MATCH_STATS_PATH', 'FORM_MATCH_AUDIT_PATH'):
        ...
        for repeat in range(args.repeats):
            ...
            for mode in modes:
                ...
                os.environ['FORM_CUDA_MATCH_REUSE'] = mode if mode in ('off', 'audit', 'certified') else 'off'
                if args.diagnostic and mode not in ('baseline', 'cpu'):
                ...
                argv = [str(args.baseline.resolve() if mode == 'baseline' else binary),
                        ...
                        '--backend', 'cpu-extraction' if mode == 'cpu' else 'cuda-extraction',
                ...
                if args.diagnostic and mode not in ('baseline', 'cpu'):
                    ...
                    if mode == 'audit' and any(int(row['oracle_searched']) != int(row['total']) for row in stats):
