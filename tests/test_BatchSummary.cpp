...
TEST(BatchSummary, ReusesResidentConfigurationAcrossRootAndTopologyUpdates) {
#ifndef FORM_ENABLE_CUDA
  GTEST_SKIP() << "CUDA disabled";
#else
  struct Enable { bool old=form::diagnostics::enabled; Enable(){form::diagnostics::enabled=true;} ~Enable(){form::diagnostics::enabled=old;} } enabled;
  using namespace form::diagnostics;
  auto count=[] {return calls[static_cast<size_t>(Stage::cuda_configure_host)].load();};
  auto root=std::make_shared<form::FeatureSummary>(Eigen::Matrix<double,13,13>::Identity(),Eigen::Matrix<double,7,7>::Identity());
  auto other=std::make_shared<form::FeatureSummary>((2*Eigen::Matrix<double,13,13>::Identity()).eval(),Eigen::Matrix<double,7,7>::Zero());
  Eigen::MatrixXd h=Eigen::MatrixXd::Identity(7,7);h(6,6)=10.;
  std::vector<form::FrozenSystem> frozen={{{1},{h.data(),h.data()+h.size()},true}};
  std::vector<gtsam::Pose3> poses(3);poses[2]=gtsam::Pose3(gtsam::Rot3{}, {1,2,3});
  std::vector<double> deltas(6,.02);
  form::BatchSummary batch(3,{{0,2,root,1.}},true);
  batch.configureResident(frozen,{});
  int trial=0;
  for (auto edges : {std::vector<form::SummaryEdge>{{0,2,other,.5}},
                     std::vector<form::SummaryEdge>{{1,2,root,2.}},std::vector<form::SummaryEdge>{}}) {
    batch.residentLinearize(poses,deltas,{});
    const auto topology_before=calls[static_cast<size_t>(Stage::cuda_topology_reuse)].load();
    batch.reset(3,edges);
    EXPECT_EQ(calls[static_cast<size_t>(Stage::cuda_topology_reuse)].load(),topology_before+(trial==0 ? 1 : 0));
    ++trial;
    EXPECT_THROW(batch.residentHessian(),std::logic_error);
    // Successful reset retains dimension-compatible constants even before configure.
    EXPECT_NO_THROW(batch.residentLinearize(poses,deltas,{}));
    const auto before=count();batch.configureResident(frozen,{});
    EXPECT_EQ(count(),before);
    EXPECT_THROW(batch.residentHessian(),std::logic_error);
    batch.residentLinearize(poses,deltas,{});
    form::BatchSummary fresh(3,edges,true);fresh.configureResident(frozen,{});
    fresh.residentLinearize(poses,deltas,{});
    EXPECT_TRUE(batch.residentHessian().isApprox(fresh.residentHessian(),1e-13));
    EXPECT_DOUBLE_EQ(batch.residentError(poses,deltas),fresh.residentError(poses,deltas));
  }
  // Changed auxiliary layout of the same size must rebuild its mapping.
  for (int pose : {0,2}) {
    const auto before=count();batch.configureResident(frozen,{{pose}});EXPECT_EQ(count(),before+1);
    std::vector<double> aux(h.data(),h.data()+h.size());
    batch.residentLinearize(poses,deltas,aux);
    form::BatchSummary fresh(3,{},true);fresh.configureResident(frozen,{{pose}});
    fresh.residentLinearize(poses,deltas,aux);
    EXPECT_TRUE(batch.residentHessian().isApprox(fresh.residentHessian(),1e-13));
  }
  auto invalid=frozen;invalid[0].augmented.pop_back();
  EXPECT_THROW(batch.configureResident(invalid,{{2}}),std::invalid_argument);
  EXPECT_THROW(batch.residentLinearize(poses,deltas,std::vector<double>(49)),std::logic_error);
  const auto before=count();batch.configureResident(frozen,{{2}});EXPECT_EQ(count(),before+1);
  batch.residentLinearize(poses,deltas,std::vector<double>(49));
  batch.reset(4,{});
  EXPECT_THROW(batch.residentLinearize(std::vector<gtsam::Pose3>(4),deltas,std::vector<double>(49)),std::logic_error);
#endif
}
